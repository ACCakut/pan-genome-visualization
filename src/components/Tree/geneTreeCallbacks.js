...
const onTipsHover = function (d) {
  d.elem
    .attr('r', function (x) {
      return x.tipAttributes.r * 2
    })
    .style('fill', function (x) {
      return subtree_node_colors(x.n.accession)
    })
  d.strainTip
    .attr('r', function (x) {
      return x.tipAttributes.r * 2
    ...
    .style('fill', function (x) {
      return subtree_node_colors(x.n.name)
    })
...
const onTipHover = function (d) {
  ...
  for (var gi = 0, len = d.paralogs.length; gi < len; gi++) {
    d.paralogs[gi].elem
      ...
      .style('fill', function (x) {
        return d3.rgb(x.tipAttributes.fill).darker(panXTree.strokeToFill).toString()
      }) //"#81E"
  }
}
...
const onBranchHover = function (d) {
  d3.select('#coreTree_legend').style('visibility', 'hidden')
  if (d.n.children == undefined) {
    onTipHover(d)
  } else {
    preOrderIteration(d, function (x) {
      if (x.terminal) {
        onTipsHover(x)
      }
    })
...
const onBranchLeave = function (d) {
  d3.select('#coreTree_legend').style('visibility', 'visible')
  preOrderIteration(d, function (x) {
    if (x.terminal) {
      onTipLeave(x)
    }
